import React from 'react'



const Footer = () => {
    return (
        <footer >
             
            <p>Created by zbyszom, © 2019</p><br/>
           <div className = 'social'> <a href="https://www.facebook.com/profile.php?id=100004800559124" target="_blank" rel="noopener noreferrer"><i className="fab fa-facebook fa-2x "></i></a>
        
        <a href="https://www.youtube.com/user/zbyszrom/videos?view_as=subscriber" target="_blank" rel="noopener noreferrer"><i className="fab fa-youtube fa-2x "></i></a> 
        </div>  
        </footer>
    )
}

export default Footer