...
const Polish = (props) => {
    ...
    const prevPage = currentPage - 1 === 1 ? '/lcategory/pl' : `/lcategory/pl/${currentPage - 1}`
    const nextPage = `/lcategory/pl/${currentPage + 1}`
  ...
  query PolishQuery($skip: Int!, $limit: Int!)  {
    allContentfulLyric(
     filter: {categoryLyrics: {slug: {eq: "pl"}}}
     sort: { fields: artists }
     skip: $skip
     limit: $limit
           ) {
